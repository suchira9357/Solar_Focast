...
try:
    ...
    from numba.core.errors import NumbaWarning  # FIXED: Proper import for NumbaWarning
...
class HyperOptimizedWindField:
    ...
    def _build_static_cache(self):
        ...
        grid_coords = {
            ...
            'domain_to_grid': (grid_size - 1.01) / self.config.domain_size
        }
    ...
    def sample(self, x: float, y: float, z: Optional[float] = None) -> Tuple[float, float]:
        """Ultra-optimized sampling with aggressive caching."""
        if z is None:
            z = 0.5
